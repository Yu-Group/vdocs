...
1. In the parameter section of the PCS lab notebook (i.e., the `params:` section within the yaml header of the Rmarkdown), input the desired values in the `value:` slot for each parameter.
2. Alternatively, when it is time to knit the document, click on the knit dropdown menu, and click `Knit with parameters`. A pop-up box will show up with slots to fill out for each parameter.
...
After answering the guided questions and knitting the Rmarkdown, we get a beautiful html report like the TCGA BRCA report [here](), generated from the Rmarkdown file [here](). If a pdf is preferred over html, one can click `Knit to pdf_document` in the knit dropdown menu.
