...
#' @param dist_matrix_x Distance matrix for clustering rows (optional). Must 
...
plotHeatmap <- function(X, y_groups = NULL, x_groups = NULL,
  ...
  if (ytext_num) {
    ...
    X_long <- X_long %>%
      dplyr::mutate(y = forcats::fct_rev(forcats::fct_inorder(ytext_labels))) %>%
      tidyr::gather(key = "x", value = "fill", -y) 
  }
  if (xtext_num) {
  ...
  get_clust_order <- function(dist_mat, dist_metric, linkage, groups,
    ...
    if (is.null(groups)) {
      ...
      for (group in unique(groups)) {
        ...
        if (sum(group_idx) > 1) {
          Dmat_sub <- as.dist(as.matrix(Dmat)[group_idx, group_idx])
          hclust_out <- hclust(Dmat_sub, method = linkage)
