...
#'
#' @export
get_data_dimensions <- function(Xtrain, Xvalid, Xtest, print = TRUE) {
  text_out <- paste(
...
#'
#' @export
get_data_types <- function(X, y, html = knitr::is_html_output(), ...) {
...
#'
#' @export
get_data_summary <- function(X, y = NULL, skim_out = NULL,
                             digits = 2, sigfig = FALSE,
  ...
  dtypes <- c("factor", "numeric", "character", "logical", "complex", "Date",
              "POSIXct")
  for (dtype in unique(skim_out$skim_type)) {
    skim_df <- skim_out %>%
    ...
    if (dtype %in% dtypes) {
      ...
      skim_df <- skim_df %>% dplyr::select(tidyselect::all_of(keep_cols))
    }
