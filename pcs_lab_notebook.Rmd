...
### Data Split Overview {.tabset .tabset-pills .tabset-fade}
...
model_errs_b %>%
  prettyDT(digits = 2, sigfig = F, rownames = FALSE, 
           ...
           bold_function = ". == max(., na.rm = TRUE)", bold_margin = 2,
           bold_scheme = c(F,T,F,T,F),#c(F, rep(T, ncol(.) - 1)),
           options = list(dom = 't'))

# boxplots
data_plot = data.frame(model_errs_b_tmp[1:2], stack(model_errs_b_tmp[3:ncol(model_errs_b_tmp)]))
fnt = 12
fnt2=12
ggplot(data_plot, aes(x=model, y=values)) + 
  theme_bw() + 
...
#### tidymodels {.unnumbered}
```{r tidymodels-fits-stability, eval = TRUE}
...
```
#### tidymodels {.unnumbered}
```{r tidymodels-finalfits, eval = TRUE}
# TODO: add code for tuning parameters
mod_recipe <- recipes::recipe(.y ~., data = splits)
